            ...
            onSnapshot(que, (snapshot) => {
                if (!snapshot.empty) {
                    snapshot.docs.forEach((docData) => {
                        ...
                        const url = `admin-OPCRF-search.html?data=${encodeURIComponent(employeeDocID)}`;
                ...
                onSnapshot(que, (snapshot) => {
                    if (!snapshot.empty) {
                        snapshot.docs.forEach((docData) => {
                            ...
                            // Assuming you want to add the employeeDocID to the URL without reloading
                            const url = `admin-OPCRF-search.html?data=${encodeURIComponent(receivedStringData)}&opcrfID=${ipcrfID}&retrieved_data=true`;
...
window.addEventListener('load', CheckOPCRFRecord)
...
export function SaveEmployeeIPCRF(dtrDetails) {
    ...
    Swal.fire({
        title: "Are you sure?",
        text: "The record of OPCRF for the semester will be saved on the system",
        icon: "question",
        ...
        if (docRef) {
            ...
            Swal.fire({
                ...
                text: 'OPCRF saved successfully.',
                icon: 'success',
            }).then(() => {
                console.log('OPCRF saved successfully...');
                window.location.href = `admin-OPCRF-search.html`;
            })
        }
    }).catch((error) => {
...
function SaveAttachment(docRef) {
    const storageRef = ref(storage, "OPCRF/Employee Reports");
    const RequestcolRef = collection(db, 'OPCRF Information');
        ...
        .then(() => {
            ...
            Swal.fire({
                ...
                if (docRef) {
                    Swal.fire({
                        title: 'Congratulations!',
                        text: 'OPCRF updated successfully.',
                        icon: 'success',
                    }).then(() => {
                        console.log('OPCRF saved successfully...');
                        window.location.href = `admin-OPCRF-search.html`;
            ...
            deleteButton.addEventListener('click', function () {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        const EmployeecolRef = collection(db, 'OPCRF Information');
                        ...
                        return setDoc(employeeDocRef, newattachments,  { merge: true }).then(() => {
                            ...
                            window.location.href = `admin-OPCRF-search.html?data=${encodeURIComponent(receivedStringData)}}`;
                        })
